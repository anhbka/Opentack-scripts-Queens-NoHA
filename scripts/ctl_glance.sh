...
GRANT ALL PRIVILEGES ON glance.* TO 'glance'@'$CTL1_IP_NIC3' IDENTIFIED BY '$PASS_DATABASE_GLANCE';
FLUSH PRIVILEGES;"
...
function glance_user_endpoint() {
        ...
        openstack endpoint create --region RegionOne image public http://$CTL1_IP_NIC3:9292
        openstack endpoint create --region RegionOne image internal http://$CTL1_IP_NIC3:9292
        openstack endpoint create --region RegionOne image admin http://$CTL1_IP_NIC3:9292
}
...
function glance_install_config() {
        ...
        ###glance_api_conf
		ops_edit $glance_api_conf DEFAULT transport_url rabbit://openstack:$RABBIT_PASS@$CTL1_IP_NIC3
		ops_edit $glance_api_conf DEFAULT bind_host = $CTL1_IP_NIC3
        ...
        ops_edit $glance_api_conf database connection mysql+pymysql://glance:$PASS_DATABASE_GLANCE@$CTL1_IP_NIC3/glance

        ops_edit $glance_api_conf keystone_authtoken auth_uri http://$CTL1_IP_NIC3:5000		
        ops_edit $glance_api_conf keystone_authtoken auth_url http://$CTL1_IP_NIC3:5000		
        ops_edit $glance_api_conf keystone_authtoken memcached_servers $CTL1_IP_NIC3:11211
		...
		ops_edit $glance_registry_conf DEFAULT transport_url rabbit://openstack:$RABBIT_PASS@$CTL1_IP_NIC3
		ops_edit $glance_registry_conf DEFAULT bind_host = $CTL1_IP_NIC3
        ops_edit $glance_registry_conf database connection mysql+pymysql://glance:$PASS_DATABASE_GLANCE@$CTL1_IP_NIC3/glance

        ops_edit $glance_registry_conf keystone_authtoken auth_uri http://$CTL1_IP_NIC3:5000		
        ops_edit $glance_registry_conf keystone_authtoken auth_url http://$CTL1_IP_NIC3:5000		
        ops_edit $glance_registry_conf keystone_authtoken memcached_servers $CTL1_IP_NIC3:11211
...
echocolor "Creat DB Glance"
sleep 3
glance_create_db

echocolor "Creat user va endpoint cho Glance"
sleep 3

...
function com_config_nova {
	 ...
	 ops_edit $com_nova_conf DEFAULT my_ip $(ip addr show dev ens35 scope global | grep "inet " | sed -e 's#.*inet ##g' -e 's#/.*##g')
	 ...
	 ops_edit $com_nova_conf keystone_authtoken auth_url http://$CTL1_IP_NIC3:500/v3
	 ops_edit $com_nova_conf keystone_authtoken memcached_servers $CTL1_IP_NIC3:11211
	 ...
	 ops_edit $com_nova_conf vnc enable True
	 ops_edit $com_nova_conf server_listen 0.0.0.0
	 ops_edit $com_nova_conf server_proxyclient_address \$my_ip
	 ops_edit $com_nova_conf novncproxy_base_url http://$CTL1_IP_NIC3:6080/vnc_auto.html
	 
	 ops_edit $com_nova_conf glance api_servers http://$CTL1_IP_NIC3:9292
	 ...
	 ops_edit $com_nova_conf placement auth_type http://$CTL1_IP_NIC3:5000/v3
	 ops_edit $com_nova_conf placement username placement
	 ops_edit $com_nova_conf placement password $PLACEMENT_PASS
	 
	 ops_edit $com_nova_conf neutron url http://$CTL1_IP_NIC3:9696
	 ops_edit $com_nova_conf neutron http://$CTL1_IP_NIC3:5000
...
function com_neutron_config {
	 ...
	 ops_edit $com_neutron_conf DEFAULT transport_url rabbit://openstack:$RABBIT_PASS@$CTL1_IP_NIC3
	 ...
	 ops_edit $com_neutron_conf oslo_messaging_rabbit rabbit_host $CTL1_IP_NIC3
	 ...
	 ops_edit $com_neutron_conf keystone_authtoken auth_uri http://$CTL1_IP_NIC3:5000
	 ops_edit $com_neutron_conf keystone_authtoken auth_url http://$CTL1_IP_NIC3:5000
	 ops_edit $com_neutron_conf keystone_authtoken memcached_servers $CTL1_IP_NIC3:11211
	 ...
	 ops_edit $com_linuxbridge_agent linux_bridge physical_interface_mappings provider:ens35
	 ...
	 ops_edit $com_metadata_agent DEFAULT nova_metadata_host $CTL1_IP_NIC3

...
function nova_user_endpoint {
	...
	openstack endpoint create --region RegionOne compute public http://$CTL1_IP_NIC3:8774/v2.1/%\(tenant_id\)s
	openstack endpoint create --region RegionOne compute internal http://$CTL1_IP_NIC3:8774/v2.1/%\(tenant_id\)s
	openstack endpoint create --region RegionOne compute admin http://$CTL1_IP_NIC3:8774/v2.1/%\(tenant_id\)s

	openstack user create placement --domain default --password $PLACEMENT_PASS
	openstack role add --project service --user placement admin
	openstack service create --name placement --description "Placement API" placement
	openstack endpoint create --region RegionOne placement public http://$CTL1_IP_NIC3:8778
	openstack endpoint create --region RegionOne placement internal http://$CTL1_IP_NIC3:8778
	openstack endpoint create --region RegionOne placement admin http://$CTL1_IP_NIC3:8778
...
function nova_config {
        ...
        ops_edit $ctl_nova_conf DEFAULT transport_url rabbit://openstack:$RABBIT_PASS@$CTL1_IP_NIC3
				
        ops_edit $ctl_nova_conf DEFAULT my_ip $CTL1_IP_NIC3
        ...
        ops_edit $ctl_nova_conf api_database connection  mysql+pymysql://nova:$PASS_DATABASE_NOVA_API@$CTL1_IP_NIC3/nova_api
        ops_edit $ctl_nova_conf database connection  mysql+pymysql://nova:$PASS_DATABASE_NOVA@$CTL1_IP_NIC3/nova
				
        ops_edit $ctl_nova_conf api auth_strategy  keystone

        ops_edit $ctl_nova_conf keystone_authtoken www_authenticate_uri http://$CTL1_IP_NIC3:5000		
        ops_edit $ctl_nova_conf keystone_authtoken auth_url http://$CTL1_IP_NIC3:5000
        ops_edit $ctl_nova_conf keystone_authtoken memcached_servers $CTL1_IP_NIC3:11211
        ...
        ops_edit $ctl_nova_conf glance api_servers http://$CTL1_IP_NIC3:9292
        ...
        ops_edit $ctl_nova_conf placement auth_url http://$CTL1_IP_NIC3:5000/v3
        ops_edit $ctl_nova_conf placement username placement
        ops_edit $ctl_nova_conf placement password $PLACEMENT_PASS
        
        ops_edit $ctl_nova_conf neutron url http://$CTL1_IP_NIC3:9696
        ops_edit $ctl_nova_conf neutron auth_url http://$CTL1_IP_NIC3:5000

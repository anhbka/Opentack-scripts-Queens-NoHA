...
function neutron_user_endpoint() {
        ...
        openstack endpoint create --region RegionOne network public http://$CTL1_IP_NIC3:9696
        openstack endpoint create --region RegionOne network internal  http://$CTL1_IP_NIC3:9696
        openstack endpoint create --region RegionOne network admin  http://$CTL1_IP_NIC3:9696
}
...
function neutron_config() {
        ...
        ops_edit $ctl_neutron_conf DEFAULT transport_url rabbit://openstack:$RABBIT_PASS@$CTL1_IP_NIC3
        ...
        ops_edit $ctl_neutron_conf database connection  mysql+pymysql://neutron:$PASS_DATABASE_NEUTRON@$CTL1_IP_NIC3/neutron

        ops_edit $ctl_neutron_conf keystone_authtoken auth_uri http://$CTL1_IP_NIC3:5000
        ops_edit $ctl_neutron_conf keystone_authtoken auth_url http://$CTL1_IP_NIC3:5000
        ops_edit $ctl_neutron_conf keystone_authtoken memcached_servers $CTL1_IP_NIC3:11211
        ...
        ops_edit $ctl_neutron_conf nova auth_url http://$CTL1_IP_NIC3:5000
        ...
        ops_edit $ctl_openvswitch_agent ovs local_ip $(ip addr show dev ens34 scope global | grep "inet " | sed -e 's#.*inet ##g' -e 's#/.*##g')
        ...
        ops_edit $ctl_metadata_agent DEFAULT nova_metadata_ip $CTL1_IP_NIC3

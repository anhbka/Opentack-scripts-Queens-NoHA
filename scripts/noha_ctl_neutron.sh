...
function neutron_config() {		
    ...
    ctl_metadata_agent=/etc/neutron/metadata_agent.ini
        ...
        ops_edit $ctl_neutron_conf linux_bridge physical_interface_mappings provider:ens33
        ...
        ops_edit $ctl_linuxbridge_agent linux_bridge physical_interface_mappings provider:ens33
        ops_edit $ctl_linuxbridge_agent vxlan enable_vxlan True
        ops_edit $ctl_linuxbridge_agent vxlan local_ip $(ip addr show dev ens34 scope global | grep "inet " | sed -e 's#.*inet ##g' -e 's#/.*##g')
